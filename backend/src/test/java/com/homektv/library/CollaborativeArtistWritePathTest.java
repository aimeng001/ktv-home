...
class CollaborativeArtistWritePathTest {
    ...
    @Test
    void reparsingDoesNotOverwriteManuallyLockedIdentity() {
        Song song = song(7L, "人工歌名", "人工歌手");
        song.lockMetadata("title");
        song.lockMetadata("artist");
        SongFile file = new SongFile();
        file.setSongId(7L);
        file.setFilePath("/source/韩红-望-国语-流行.mkv");
        SongRepository songs = mock(SongRepository.class);
        SongFileRepository files = mock(SongFileRepository.class);
        ArtistCreditService credits = mock(ArtistCreditService.class);
        ...
        when(songs.findByFingerprint(anyString())).thenReturn(Optional.empty());
        when(songs.save(any(Song.class))).thenAnswer(invocation -> invocation.getArgument(0));
        when(files.findBySongIdAndValidTrueOrderByPriorityDesc(7L)).thenReturn(List.of(file));

        new SongReparseService(songs, files, credits).apply(List.of(7L), "artist_title");

        org.assertj.core.api.Assertions.assertThat(song.getTitle()).isEqualTo("人工歌名");
        org.assertj.core.api.Assertions.assertThat(song.getArtist()).isEqualTo("人工歌手");
        verify(credits).replace(7L, "人工歌手");
    }
