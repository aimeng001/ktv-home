...
class MediaApi(
    ...
    private val detailTransport: KtvTransport = KtvHttpTransport(config),
) : Closeable {
    ...
    private val playbackResolver = PlaybackResolver(detailTransport, json, { config.apiBase() }, fileSourceResolver)
    private val closed = AtomicBoolean(false)

    override fun close() {
        if (closed.compareAndSet(false, true)) {
            http.closeResources()
            (detailTransport as? Closeable)?.close()
        }
    }
    ...
    internal suspend fun resolvePlayback(songId: Long, forceTranscode: Boolean = false): PlaybackResolution =
        playbackResolver.resolve(songId, forceTranscode)
